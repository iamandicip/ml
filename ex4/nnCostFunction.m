...
for k = 1:num_labels
     J = J + (1 / m) * ...
         sum(-(y == k) .* log(h(:, k)) - (1 - (y == k)) .* log(1 - h(:, k)));
end
...
%vectorized implementation of backprop
%backprop step 1 - lines 69 - 75

%backprop step 2 - calculate d3
d3 = zeros(num_labels, m);
for k = 1:num_labels
   d3(k, :) = h(:, k) - (y == k);
end

%backprop step 3 - calculate d2
d2_temp = (Theta2' * d3);
...
% loop implementation of backprop
% for i = 1:m
%     %backprop step 1
%     a1 = [1 X(i, :)];
%     z2 = a1 * Theta1';
